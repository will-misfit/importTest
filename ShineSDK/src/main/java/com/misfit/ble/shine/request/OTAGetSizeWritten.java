...
public class OTAGetSizeWritten extends Request {
	...
	public void buildRequest() {
		byte operation = Constants.FILE_CONTROL_OPERATION_OTA_GET_WRITTEN_SIZE;
	...
	protected Response parseResponse(byte[] bytes) {
		...
		response.result = validateResponse(bytes, Constants.FILE_CONTROL_OPERATION_OTA_GET_WRITTEN_SIZE_RESPONSE);
		response.status = 0;
		...
		if (response.result == Constants.RESPONSE_SUCCESS) {
			if (bytes.length < 8) {
				...
				response.sizeWritten = byteBuffer.getInt(4);
	...
	public JSONObject getResponseDescriptionJSON() {
		...
		try {
			if (mResponse != null) {
				...
				json.put("sizeWritten", mResponse.sizeWritten);
			}
