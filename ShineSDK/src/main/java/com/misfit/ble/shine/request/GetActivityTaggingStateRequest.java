...
public class GetActivityTaggingStateRequest extends Request {
	public static class Response extends BaseResponse {
		public boolean activityTaggingState;
	}
	...
	public void handleResponse(String characteristicUUID, byte[] bytes) {
		...
		if (response.result == Constants.RESPONSE_SUCCESS) {
			if (bytes.length < 3) {
				...
				response.activityTaggingState = (byteBuffer.get(2) != 0);
			}
		}
		mResponse = response;
		mIsCompleted = true;
	...
	public JSONObject getResponseDescriptionJSON() {
		...
		try {
			if (mResponse != null) {
				...
				json.put("activityTaggingState", mResponse.activityTaggingState);
			}
