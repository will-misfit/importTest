 ...
 */
public class GetInactivityNudgeRequest extends Request {

	private static final String TAG = "GetInactivityNudgeReq";
	...
	public static class Response extends BaseResponse {
		...
		public short mRepeatIntervalMinutes;
	}
	...
	public void handleResponse(String characteristicUUID, byte[] bytes) {
		...
		if (response.result == Constants.RESPONSE_SUCCESS) {
			Log.d(TAG, "Length byte: " + bytes.length);
			if (bytes.length < 13) {
				...
				response.mRepeatIntervalMinutes = Convertor.unsignedByteToShort(byteBuffer.get(12));
			}
		}
		mResponse = response;
		mIsCompleted = true;
	...
	public JSONObject getResponseDescriptionJSON() {
		...
		try {
			if (mResponse != null) {
				...
				json.put("repeatIntervalMinutes", mResponse.mRepeatIntervalMinutes);
			}
