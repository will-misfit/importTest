...
unsigned int CalcCrc32(int len, unsigned char *pBuf);
unsigned char* as_unsigned_char_array(JNIEnv *env, jbyteArray array);

JNIEXPORT jint JNICALL Java_com_misfit_ble_util_CRCCalculator_crc(JNIEnv *env, jobject thisObj, jlong length, jbyteArray bytes) {
    return CalcCrc32((int)length, as_unsigned_char_array(env, bytes));
}
...
uint32_t CalcCrc32(int len, uint8_t *pBuf)
{
    ...
    while (len > 0)
    {
        crc = table[*pBuf ^ (uint8_t)crc] ^ (crc >> 8);
        pBuf++;
        len--;
    }
